(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function rangeInit() {
            const priceSlider = document.querySelector("#range-filter");
            if (priceSlider) {
 ...
 */        var MEDIA_PREFERS_REDUCED_MOTION = "(prefers-reduced-motion: reduce)";
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            const arrowsSlider = document.querySelectorAll(".splide--arrows");
            arrowsSlider.forEach(((slider, index) => {
                ...
                if (slider.classList.contains("bundle-product__slider")) options = {
                    ...options,
                    padding: {
                        right: "120px"
                    },
                ...
                const splideInstance = new splide_esm_Splide(slider, options);
            ...
            const bannersSliders = document.querySelectorAll(".splide--banners");
            bannersSliders.forEach((slider => {
            ...
            if (mainSliderElement) {
                var mainSlider = new splide_esm_Splide(mainSliderElement, {
                    type: "fade",
                    pagination: false,
                    arrows: true,
                    breakpoints: {
                ...
                if (thumbnailSliderElement) {
                    var thumbnailSlider = new splide_esm_Splide(thumbnailSliderElement, {
                        isNavigation: true,
                        gap: 30,
                        pagination: false,
                        arrows: false,
        ...
        da.init();
        if (document.querySelector("[data-catalog-btn]")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest("[data-catalog-btn]")) if (!document.documentElement.classList.contains("catalog-open")) {
        ...
        if (document.querySelector("[data-settings-btn]")) document.addEventListener("click", (function(e) {
            const settingsBtnEl = e.target.closest("[data-settings-btn]");
            if (settingsBtnEl) {
                const cookiesBlock = settingsBtnEl.closest(".cookies-popup");
                const settingsBody = cookiesBlock.querySelector(".cookies-popup__settings");
                _slideToggle(settingsBody);
            }
        }));
        ...
        inputs.forEach(((input, index) => {
            ...
            input.name = `product-${groupNumber}`;
            if (index % 2 === 0) input.checked = true;
        }));
        const showMoreContentElements = document.querySelectorAll("[data-showmore-content]");
        showMoreContentElements?.forEach((contentElement => {
            const showMoreButton = contentElement.nextElementSibling;
            if (showMoreButton && showMoreButton.hasAttribute("data-showmore-button")) {
                const itemsCount = contentElement.children.length;
                var overalCount = itemsCount - contentElement.dataset.showmoreContent;
                const firstSpan = showMoreButton.querySelector("span");
                if (firstSpan) firstSpan.textContent += ` + ${itemsCount - contentElement.dataset.showmoreContent}`;
                if (overalCount <= 0) showMoreButton.style.display = "none";
            }
        }));
        if (document.querySelector("[data-filter]")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && (e.target.closest("[data-filter]") || e.target.closest("[data-filter-close]"))) {
